...
import com.shaikhomes.anyrent.ui.home.LeadAdapter
...
class ViewLeadsFragment : Fragment() {
    private var _binding: FragmentViewleadsBinding? = null
    ...
    private var viewLeadsViewModel: ViewLeadsViewModel? = null
    private var leadAdapter: LeadAdapter? = null
    var propertyData: PropertyData? = null
    var pickImg1: AppCompatImageView? = null
    var pickImg2: AppCompatImageView? = null
    ...
    override fun onCreateView(
        ...
        _binding = FragmentViewleadsBinding.inflate(inflater, container, false)
        val root: View = binding.root
        viewLeadsViewModel = ViewModelProvider(this).get(ViewLeadsViewModel::class.java)
        getProperties()
    ...
    private fun getLeads(leadType: String?) {
        viewLeadsViewModel?.getLeads("", leadType!!, success = {
            viewLifecycleOwner.lifecycleScope.launch {
                delay(1000)
                if (binding.swipeLead.isRefreshing) binding.swipeLead.isRefreshing = false
                if (it.leadsList.isNotEmpty()) {
                    if (leadType == "Rent") {
                        binding.rentToggle.setText("Rent (${it.leadsList.size})")
                    } else if (leadType == "Buy") {
                        binding.buyToggle.setText("Buy (${it.leadsList.size})")
                    } else if (leadType == "Sell") {
                        binding.sellToggle.setText("Sell (${it.leadsList.size})")
                    }
    ...
    private fun showBroadCast() {
        ...
        pickImg1 = dialog.findViewById(R.id.pickImg1)
        pickImg2 = dialog.findViewById(R.id.pickImg2)
        pickImg1?.setOnClickListener {
            if (ImagePicker.isPermissionRequired(requireContext(), this))
                ImagePicker.startImagePicker(requireContext(), this)
        }
        btnSend.setOnClickListener {
            if (edtMsg.text.toString().isNotEmpty()) {
                dialog.dismiss()
                sendBroadcastMessage(edtMsg.text.toString().trim(), imageUri1)
    ...
    override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
        ...
        if (requestCode == ImagePicker.IMAGE_PICKER_REQUEST_CODE &&
            ...
            val imageUri = ImagePicker.parseImageUri(requireContext(), data) ?: Uri.EMPTY
            imageUri1 = imageUri
            FileUtil.getPath(requireContext(), imageUri).let {
                file = File(it)
                if (!file?.absolutePath.isNullOrEmpty()) {
                    pickImg1?.setImageBitmap(BitmapFactory.decodeFile(file?.absolutePath))
                }
            }
    ...
    private fun sendBroadcastMessage(message: String, imageUri1: Uri?) {
